...
const selectedLocation = urlParams.get('location'); // e.g., 'rexburg' or 'idaho falls'

let normalizedLocation = null;

// Normalize common inputs
if (selectedLocation) {
    const l = selectedLocation.toLowerCase().replace(/\s+/g, '');
    if (l === "rexburg") {
        normalizedLocation = "rexburg";
    } else if (l === "idahofalls") {
        normalizedLocation = "idahofalls";
    }
}
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (locationDropdown && normalizedLocation) {
        locationDropdown.value = normalizedLocation;

        // Dispatch a change event so your existing event listener runs
