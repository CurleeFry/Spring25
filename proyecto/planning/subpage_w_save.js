...
// Function to save current labor data
function saveCurrentData(currentLaborData) {
  const today = getTodayDateString();
  const index = savedData.findIndex(entry => entry.date === today);
  ...
  if (index !== -1) {
    savedData[index] = newEntry; // Replace existing
  } else {
    savedData.push(newEntry); // Add new
  }

  console.log('Saved data for today:', newEntry);
}
...
if (saveButton) {
  saveButton.addEventListener('click', () => {
    // Replace `currentLaborData` with your actual parsed data variable
    if (typeof currentLaborData !== 'undefined') {
      saveCurrentData(currentLaborData);
    } else {
      console.warn('No labor data available to save.');
    }
  });
} else {
  console.warn('Save button with id "saveBtn" not found on the page.');
}